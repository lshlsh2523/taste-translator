...
- 이 경우 만점 기준도 6점이 아니라 **5점**으로 조정됩니다(형태 신호가
  빠진 만큼). \`total_score\`는 이 5점 기준 그대로 반환하세요 — 6점
  만점으로 환산하지 마세요.
...
# 채점 결과 반환 (수정본 — 임계값 판단은 이 프롬프트가 아니라 이후
# 오케스트레이션 코드가 함)

- 후보 전체를 채점한 뒤, \`total_score\`가 높은 순으로 최대 6개까지
  \`recommended_products\`에 포함하세요. **점수가 낮다는 이유로 후보를
  미리 걸러내지 마세요** — 임계값을 넘겼는지 여부는 이 단계의 책임이
  아닙니다. 점수를 정직하게 매기는 것까지만 이 단계의 역할입니다.
- \`no_product_match\`는 candidate_products 자체가 비어있어 채점할
  대상이 아예 없을 때만 true로 설정하세요(이 프롬프트는 이미 최소
  1개 이상의 후보를 받고 호출되므로 사실상 발생하지 않습니다).
